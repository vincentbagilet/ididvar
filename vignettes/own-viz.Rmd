...
country_weights |> 
  mutate(country = forcats::fct_reorder(country, contrib)) |>
  ggplot(aes(x = contrib, y = country)) + 
  geom_col(fill = "#300D49") +
  theme_idid() +
  labs(
    title = "Distribution of contributing observations",
    x = "Share of contributing years",
    y = NULL
  ) 
```
...
idid_viz_contrib(reg, "l_gdpPercap", var_y = country, order = "y") +
  theme_idid() +
  labs(
    title = "Distribution of contributing observations",
    x = "Share of contributing years",
...
idid_viz_weights(reg, "l_gdpPercap", var_y = country, var_x = decade, order = "y") +
  facet_grid(continent ~ ., scales = "free_y", space = "free_y") +
  theme(strip.text.y = element_text(angle = 0)) +
  labs(
    title = "Distribution of Identifying Variation Weights",
