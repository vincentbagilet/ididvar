...
1. **Visualize groups' weights** (with `idid_viz_weights` or `idid_viz_weights_map`)
1. Repeat this analysis to **identify contributing observations**, *ie* observations that we can drop without too much altering the estimate.
...
idid_viz_weights(reg_ex, "Income", pop_quartiles, state)
```
