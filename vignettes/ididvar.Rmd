...
The present vignette quickly **presents a typical workflow for analysis**, using a very basic example. It also provides an **overview of most [the functions](reference/index.html) available** in the package. 
...
1. Compute the **observation level weights** and add a `weight` variable to the data set (using the `idid_weights` function)
1. **Visualize** the distribution of these weights (the cumulative one with `idid_viz_cumul`, the observation level one with `idid_viz_weights` or `idid_viz_weights_map`). One might already get a sense of the effective sample
1. **Identify groups** for which there is a large heterogeneity in weights across groups (with `idid_grouping_var`)
1. Visualize the weights of these groups (with `idid_viz_weights` or `idid_viz_weights_map`)
1. Repeat this analysis to identify **observations that we can drop without too much altering the estimate**.
...
We can then visualize these weights. 
